<template>
  <div  :style="{ width: windowWidth + 'px', height: windowHeight + 'px' }" class="udp-server-view">
    <div class="header">
...
import { GetUdpServerData,GetUdpServerStatus, StartUdpServer, StopUdpServer, SendMessage, DisconnectClient } from "../../wailsjs/go/control/FuncUdpServer"
import { GetAllUdpServerConn } from "../../wailsjs/go/control/UdpServerConn"
...
export default {
  ...
  mounted() {
    ...
    EventsOn('server_event', (event) => {
      ...
      console.log("server_event", event.message.conn_id , this.clientConnection)
  ...
  methods: {
    ...
    async checkConnectionStatus() {
      try {
        const response = await GetUdpServerStatus(this.serverData.id)
    ...
    async handleConnection() {
      try {
        ...
        if (this.isConnected) {
          const response = await StopUdpServer(this.serverData.id)
          ...
          this.connecting = true
          const response = await StartUdpServer(this.serverData.id) 
          if (response && response.success) {
    ...
    async handleEdit(id) {
      try {
        const response = await GetUdpServerData(id)
        if (response && response.data) {
    ...
    async sendMessage(port) {
      ...
      try {
        console.log("11111111111",this.serverData)
    ...
    async loadClientConnections() {
      try {
        const response = await GetAllUdpServerConn(this.serverData.id) // 需要实现这个 API
        if (response && response.data) {
