  ...
  <div v-if="visible" class="modal-overlay">
    <div class="modal-content">
      ...
      <label>
        ...
        <input type="text" value="UDP 客户端" readonly />
      </label>
      <label>
...
import { AddUdpClient, UpdateUdpClient } from "../../wailsjs/go/control/FuncUdpClient";

export default {
  name: 'AddUdpClientModal',
  props: {
    visible: {
      type: Boolean,
      required: true
  ...
  methods: {
    ...
    async save() {
      try {
        ...
        if (this.editMode) {
          udpClient.id = this.initialData.id  // 确保包含ID
          await UpdateUdpClient(udpClient)
          window.runtime.LogInfo('更新成功')
        } else {
          await AddUdpClient(udpClient)
