  ...
  <div v-if="visible" class="modal-overlay">
    <div class="modal-content">
      ...
      <label>
        ...
        <input type="text" value="UDP 服务端" readonly />
      </label>
      <label>
...
import { AddUdpServer, UpdateUdpServer } from "../../wailsjs/go/control/FuncUdpServer";
import { ShowWarningDialog } from '../../wailsjs/go/main/App'

export default {
  name: 'AddUdpServerModal',
  props: {
    visible: {
      type: Boolean,
      required: true
  ...
  methods: {
    ...
    async save() {
      try {
        ...
        if (this.editMode) {
          udpServer.id = this.initialData.id  // 确保包含ID
          const res = await UpdateUdpServer(udpServer)
          if (res.success) {
          ...
          const res = await AddUdpServer(udpServer)
          if (res.success) {
