<template>
  <div  :style="{ width: windowWidth + 'px', height: windowHeight + 'px' }" class="udp-client-view">
    <div class="header">
...
import { GetUdpClientData, ConnectUdpClient, DisconnectUdpClient, GetUdpClientStatus, SendMessage, SendScheduledMessage, StopScheduledMessage } from "../../wailsjs/go/control/FuncUdpClient"
...
export default {
    ...
    methods: {
        ...
        async checkConnectionStatus() {
            try {
                const response = await GetUdpClientStatus(this.clientData.id)
        ...
        async handleConnection() {
            try {
                ...
                if (this.isConnected) {
                    const response = await DisconnectUdpClient(this.clientData.id)
                    if (response && response.success) {
                        ...
                        await ShowWarningDialog('提示', '断开连接失败: ' + response.message)
                    }
                } else {
                    this.connecting = true
                    const response = await ConnectUdpClient(this.clientData.id) 
                    if (response && response.success) {
                        this.isConnected = true
                        window.runtime.LogInfo('连接成功')
                    } else {
                        window.runtime.LogError('连接失败')
                        await ShowWarningDialog('提示', '连接失败: ' + response.message)
                    }
                }
            } catch (error) {
                window.runtime.LogError('连接操作失败: ' + error)
                await ShowWarningDialog('提示', '连接操作失败')
            } finally {
        ...
        // 编辑
            async handleEdit(id) {
            try {
                const response = await GetUdpClientData(id)
                if (response && response.data) {
